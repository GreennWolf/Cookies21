...
export const createTemplate = async (bannerConfig, isSystemTemplate = false) => {
  try {
    ...
    // Si es una plantilla del sistema (pública), usar el endpoint correspondiente
    if (isSystemTemplate) {
      endpoint = '/api/v1/banner-templates/system';
    }
    
    if (isFormData) {
      // Si es plantilla del sistema, agregar el flag al FormData
      ...
      const payload = {
        ...
        isSystemTemplate: isSystemTemplate // Añadir flag para plantillas del sistema
      };
