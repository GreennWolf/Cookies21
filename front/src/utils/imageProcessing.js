      ...
      if (window._imageFiles && window._imageFiles[component.content]) {
        ...
        if (typeof URL !== 'undefined' && URL.createObjectURL) {
          try {
            ...
            console.log(`✓ ${context}: Creado ObjectURL para imagen temporal: ${objectUrl.substring(0, 30)}...`);
            ...
            console.warn(`⚠️ ${context}: Error creando ObjectURL:`, err);
          }
        }
      }
...
export const saveAspectRatioToCache = (imageUrl, aspectRatio) => {
  ...
  try {
    ...
    console.log(`✅ Aspect ratio guardado en caché para ${imageUrl}: ${aspectRatio.toFixed(2)}`);
  } catch (error) {
...
/**
 * Utilidad para procesar imágenes en componentes para FormData
