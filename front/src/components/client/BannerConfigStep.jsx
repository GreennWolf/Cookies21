...
const BannerConfigStep = ({ formData, onChange, selectedDomain }) => {
  ...
  const handleToggleConfig = async (e) => {
    // Prevenir comportamiento predeterminado para evitar posibles envíos de formulario
    if (e && e.preventDefault) {
      e.preventDefault();
      e.stopPropagation();
    ...
    if (newValue && templates.length === 0) {
      ...
      try {
        ...
        // Si hay plantillas, guardarlas y continuar
    ...
    setIsConfiguring(newValue);
    
    // Actualizar formData principal sin reiniciar la configuración
    onChange('configureBanner', newValue);
    
    // Si se activa la configuración y ya tenemos configuración guardada en formData,
    // no hacemos nada más para mantener los datos previos
    if (newValue && formData.bannerConfig) {
      // Intentar cargar la plantilla si existe un templateId
      if (formData.bannerConfig.templateId && !selectedTemplate) {
      ...
      // Cargar la configuración del formData principal si existe
      if (Object.keys(formData.bannerConfig).length > 0) {
        // Preservar específicamente tipo y posición
        const preservedConfig = {
          ...formData.bannerConfig,
          bannerType: formData.bannerConfig.bannerType || bannerConfig.bannerType,
          position: formData.bannerConfig.position || bannerConfig.position
        };
        
        console.log("🧲 Cargando configuración preservada:", {
          tipo: preservedConfig.bannerType,
          posición: preservedConfig.position
        });
    ...
    // Si se desactiva la configuración, solo actualizamos el estado en el padre
    // pero mantenemos los datos locales para una posible reactivación
  };
