...
const CookieAnalytics = ({ data }) => {
  const [sortBy, setSortBy] = useState('acceptanceRate'); // 'acceptanceRate', 'rejectionRate' o 'count'
  ...
  // Ordenar cookies por tasa de aceptación, rechazo o recuento
  const sortedAcceptance = [...acceptance].sort((a, b) => {
    if (sortBy === 'acceptanceRate') {
      return b.acceptanceRate - a.acceptanceRate;
    }
    if (sortBy === 'rejectionRate') {
  ...
  const barOptions = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: `Tasas por Cookie (Top 10, ordenado por ${
          sortBy === 'acceptanceRate' ? 'aceptación' : 
          sortBy === 'rejectionRate' ? 'rechazo' : 'uso'
        })`,
        font: {
          size: 14,
          weight: 'bold'
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 rounded shadow">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-medium text-gray-600">Tasas de Aceptación y Rechazo (Top 10)</h3>
            <div className="flex space-x-2">
              <button 
                onClick={() => setSortBy('acceptanceRate')}
                className={`px-3 py-1 text-xs rounded-full ${
                  sortBy === 'acceptanceRate' 
                    ? 'bg-[rgba(75, 192, 192, 1)] text-white' 
              ...
              <button 
                onClick={() => setSortBy('rejectionRate')}
                className={`px-3 py-1 text-xs rounded-full ${
                  sortBy === 'rejectionRate' 
                    ? 'bg-[rgba(255, 99, 132, 1)] text-white' 
                    : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
                } transition-colors`}
              >
                Por Rechazo
              ...
              <button 
                ...
                className={`px-3 py-1 text-xs rounded-full ${
                  sortBy === 'count' 
                    ? 'bg-[rgba(54, 162, 235, 1)] text-white' 
                    : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
                } transition-colors`}
              >
                Por Uso
