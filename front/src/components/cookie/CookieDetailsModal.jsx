        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <ul className="list-disc ml-6">
              <li><strong>Duración:</strong> {cookie.attributes.duration}</li>
              <li><strong>Tipo:</strong> {cookie.attributes.type}</li>
              <li><strong>Path:</strong> {cookie.attributes.path}</li>
              <li><strong>Dominio:</strong> {cookie.attributes.domain}</li>
              <li><strong>Secure:</strong> {cookie.attributes.secure ? 'Sí' : 'No'}</li>
              <li><strong>HttpOnly:</strong> {cookie.attributes.httpOnly ? 'Sí' : 'No'}</li>
              <li><strong>SameSite:</strong> {cookie.attributes.sameSite}</li>
            </ul>
          </div>
          <div>
            <strong>Detección:</strong>
            <ul className="list-disc ml-6">
              <li><strong>Método:</strong> {cookie.detection.method}</li>
              <li><strong>Primera detección:</strong> {new Date(cookie.detection.firstDetected).toLocaleString()}</li>
              <li><strong>Última vez vista:</strong> {new Date(cookie.detection.lastSeen).toLocaleString()}</li>
              <li><strong>Frecuencia:</strong> {cookie.detection.frequency}</li>
              <li><strong>Patrón:</strong> {cookie.detection.pattern}</li>
            </ul>
          </div>
          <div>
          ...
          <div>
            <strong>Estado:</strong> {cookie.status}
          </div>
          <div>
            <strong>Metadata:</strong>
            <ul className="list-disc ml-6">
              <li><strong>Creado por:</strong> {cookie.metadata.createdBy}</li>
              <li><strong>Última modificación por:</strong> {cookie.metadata.lastModifiedBy}</li>
              <li><strong>Versión:</strong> {cookie.metadata.version}</li>
            </ul>
          </div>
        </div>
