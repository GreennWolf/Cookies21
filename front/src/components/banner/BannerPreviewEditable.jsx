...
function BannerPreviewEditable({ 
  ...
  const renderComponent = (component) => {
    ...
    switch (component.type) {
      ...
      case 'container': {
        // Soporte para contenedores en previews editables
        const containerConfig = component.containerConfig?.[deviceView] || {};
        const displayMode = containerConfig.displayMode || 'libre';
        
        // Estilos base del contenedor para preview editable (aplicar TODAS las configuraciones)
        ...
        const renderChildren = () => {
          ...
          return component.children.map((child) => {
            ...
            // Renderizar según el tipo de componente hijo (usando renderComponent recursivamente)
            const childComponent = renderComponent(child);
            
            // Si el hijo tiene capacidades de edición, mantenerlas
            if (child.type === 'image' && displayMode === 'libre') {
              // Para imágenes en modo libre, mantener funcionalidad drag/resize
              return childComponent;
            } else {
              // Para otros casos, envolver en div no editable
              return (
                <div key={child.id} style={finalChildStyles}>
                  {childComponent}
                </div>
              );
            }
