...
const BannerThumbnail = ({ bannerConfig, className = '', deviceView = 'desktop' }) => {
  ...
  const getThumbnailDimensions = () => {
    ...
    // Dimensiones del contenedor del thumbnail (aumentadas para mostrar banner más grande)
    const thumbWidth = containerDimensions.width * 0.90; // 90% del contenedor
    const thumbHeight = containerDimensions.height * 0.85; // 85% para aprovechar más altura
    
    // Factor de escala adicional para el tamaño del banner
    const scaleFactor = 0.9; // Mantener banner casi al tamaño completo
    
    // Proporciones del banner real según su tipo con mejores ratios para visualización
    let aspectRatio = 16/9; // Default
    let minHeight = 80; // Altura mínima en píxeles (aumentada para banner más grande)
    
    if (layout.type === 'floating') {
      aspectRatio = 4/3; // Más alto para mejor visualización
      minHeight = 100; // Aumentado para mejor visibilidad
    } else if (layout.type === 'modal') {
      aspectRatio = 3/2; // Proporción más alta para modales
      minHeight = 120; // Aumentado para mejor proporción
    } else if (layout.type === 'banner') {
      aspectRatio = 3.5/1; // Proporción para banners
      minHeight = 70; // Aumentado
    }
  ...
  const convertPercentageToPixels = (styleObj, bannerRef, scaleFactor = 0.45) => {
    ...
    try {
      ...
      // Convertir width si es porcentaje y aplicar escala
      if (converted.width && typeof converted.width === 'string' && converted.width.includes('%')) {
        ...
        converted.width = `${Math.round(pixelValue * scaleFactor)}px`;
      ...
      // Convertir height si es porcentaje y aplicar escala
      if (converted.height && typeof converted.height === 'string' && converted.height.includes('%')) {
        ...
        converted.height = `${Math.round(pixelValue * scaleFactor)}px`;
      ...
      // Convertir otras propiedades con escala
      ['maxWidth', 'minWidth', 'maxHeight', 'minHeight', 'top', 'left', 'right', 'bottom', 'padding', 'margin', 'borderRadius', 'fontSize'].forEach(prop => {
        if (converted[prop] && typeof converted[prop] === 'string') {
          if (converted[prop].includes('%')) {
            ...
            // Para posiciones, mantener el valor porcentual correcto sin escalar
            if (['top', 'left', 'right', 'bottom'].includes(prop)) {
              converted[prop] = `${pixelValue}px`;
            } else {
              // Aplicar escala a dimensiones
              converted[prop] = `${Math.round(pixelValue * scaleFactor)}px`;
            }
          } else if (converted[prop].includes('px')) {
            const pixelValue = parseFloat(converted[prop]);
            // Aplicar escala solo a propiedades que no sean posiciones
            const shouldScale = !['top', 'left', 'right', 'bottom'].includes(prop);
            converted[prop] = `${Math.round(pixelValue * (shouldScale ? scaleFactor : 1))}px`;
          }
        }
      });
  ...
  const renderComponent = (component, isChild = false) => {
    ...
    const baseStyles = isChild || component.parentId ? {
      ...
      // Para texto, mejorar legibilidad (el fontSize ya viene escalado de convertPercentageToPixels)
      ...(component.type === 'text' && {
        fontSize: convertedProcessedStyle.fontSize ? 
          `${Math.max(8, parseFloat(convertedProcessedStyle.fontSize))}px` : '8px',
        lineHeight: '1.2',
        overflow: 'hidden',
      ...
      position: 'absolute',
      // Calcular posiciones correctamente basadas en el tamaño del thumbnail
      top: devicePos.top && devicePos.top.includes('%') ? 
        `${(parseFloat(devicePos.top) / 100) * (bannerRef.current?.offsetHeight || parseFloat(getThumbnailDimensions().height))}px` : 
        (devicePos.top || '0px'),
      left: devicePos.left && devicePos.left.includes('%') ? 
        `${(parseFloat(devicePos.left) / 100) * (bannerRef.current?.offsetWidth || parseFloat(getThumbnailDimensions().width))}px` : 
        (devicePos.left || '0px'),
      ...convertedProcessedStyle,
      ...
      visibility: 'visible',
      opacity: 1,
      zIndex: component.style?.[deviceView]?.zIndex || 'auto'
    };
              ...
              {component.children && component.children.map(child => {
                ...
                if (displayMode === 'libre') {
                  ...
                  const convertedChildStyle = convertPercentageToPixels({
                    top: childPos.top,
                    left: childPos.left
                  }, bannerRef.current, 1); // No escalar posiciones de hijos dentro de contenedor
