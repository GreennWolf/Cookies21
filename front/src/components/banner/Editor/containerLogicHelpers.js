...
export const validateAddToContainer = (parentContainer, componentType, allComponents = []) => {
  ...
  // Evitar contenedores anidados demasiado profundos
  if (componentType === 'container') {
    const depth = calculateContainerDepth(parentContainer, allComponents);
    if (depth >= 5) {
      validation.isValid = false;
      validation.errors.push('Máximo nivel de anidamiento alcanzado (5 niveles)');
      return validation;
    }
    
    if (depth >= 3) {
      validation.warnings.push('Anidamiento profundo puede afectar el rendimiento');
    }
  }
