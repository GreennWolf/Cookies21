...
const LayerItem = ({ 
  ...
  const handleDragStart = useCallback((e) => {
    ...
    console.log('🎯 Drag iniciado:', component.id, 'desde índice:', arrayIndex);
  ...
  const handleDrop = useCallback((e) => {
    ...
    try {
      ...
      if (sourceRealIndex !== newRealIndex) {
        console.log('🔄 LAYERS DROP - Reordenando:', {
          componentId: dragData.componentId,
          sourceRealIndex: sourceRealIndex,
    ...
    <div className="relative">
      ...
      >
        ...
        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          {/* Eliminar - solo ocultar para componentes del sistema pero permitir para otros */}
          {!['acceptBtn', 'rejectBtn', 'preferencesBtn', 'mainText', 'titleText'].includes(component.id) && (
            <button
              onClick={(e) => {
                e.stopPropagation();
                if (window.confirm(`¿Eliminar "${displayName}"?`)) {
...
const LayersPanel = ({
  ...
  const handleReorder = useCallback((sourceIndex, targetIndex) => {
    console.log('🔄 LayersPanel handleReorder:', { 
      sourceIndex, 
      targetIndex,
      totalComponents: components.length 
    });
    onReorderComponents(sourceIndex, targetIndex);
  }, [onReorderComponents, components.length]);
