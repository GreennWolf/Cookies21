...
function BannerPropertyPanel({ 
    ...
    const checkScrollStatus = () => {
      ...
      scrollElements.forEach(element => {
        ...
        if (isScrollable && hasMoreContent) {
          needsIndicator = true;
          // Para debugging
          console.log(`Scroll disponible: ${scrollRemaining}px`);
        }
      });
  ...
  const handleContainerConfigChange = (property, value) => {
    ...
    try {
      ...
      console.log('📋 Estado de la configuración:', {
        original: component.containerConfig,
        dispositivo: deviceView,
        propiedad: property,
        valor: value,
        nuevo: newContainerConfig
      });
      
      console.log(`🔧 Actualizando configuración de contenedor - ${property}: ${value}`, newContainerConfig);
      ...
      if (typeof onUpdateContainer === 'function') {
        ...
        if (typeof window !== 'undefined') {
          ...
          console.log(`📢 Emitiendo evento container:config:update para ${component.id}:`, { property, value });
  ...
  const handleQuickPosition = (positionCode) => {
    try {
      ...
      try {
        console.log('Llamando a onUpdatePosition con:', JSON.stringify(newPosition));
      ...
      console.log(`✅ Posición actualizada a: top=${topValue}, left=${leftValue}`);
    } catch (error) {
      ...
      switch (alignment) {
        case 'center-h': {
          ...
          try {
            ...
            try {
              onUpdatePosition({ left: `${centerLeft}%` });
            } catch (fallbackError) {
          ...
          console.log(`✅ Alineación horizontal al centro: left=${centerLeft}%`);
          break;
        }
        case 'center-v': {
          ...
          try {
            ...
            try {
              onUpdatePosition({ top: `${centerTop}%` });
            } catch (fallbackError) {
              console.error('❌ Falló también el fallback:', fallbackError);
            }
          }
          ...
          console.log(`✅ Alineación vertical al centro: top=${centerTop}%`);
          break;
        }
        case 'center-both': {
          ...
          console.log(`✅ Alineación al centro: top=${centerTop}%, left=${centerLeft}%`);
          break;
        }
        case 'left': {
        ...
        case 'right': {
          ...
          try {
            ...
            // Intento alternativo sin percentX
            try {
              onUpdatePosition({ left: `${rightPos}%` });
            } catch (fallbackError) {
              console.error('❌ Falló también el fallback:', fallbackError);
            }
          }
          ...
          console.log(`✅ Alineación a la derecha: left=${rightPos}%`);
          break;
        }
        case 'top': {
          // Alinear arriba
        ...
        case 'bottom': {
          ...
          try {
            ...
            // Intento alternativo sin percentY
            try {
              onUpdatePosition({ top: `${bottomPos}%` });
            } catch (fallbackError) {
              console.error('❌ Falló también el fallback:', fallbackError);
            }
          }
          ...
          console.log(`✅ Alineación abajo: top=${bottomPos}%`);
          break;
        }
        default:
                      ...
                      if (needsRepositioning && originalRect) {
                        setTimeout(() => {
                          ...
                          if (rightEdge > canvasRect.width) {
                            ...
                            console.log(`Reposicionando horizontalmente: nuevo left=${newLeft}%`); 
                          }
                          ...
                          if (bottomEdge > canvasRect.height) {
                            ...
                            console.log(`Reposicionando verticalmente: nuevo top=${newTop}%`);
                          }
                        }, 50);
                      }
                      
                      console.log(`✅ Dimensiones autocompletadas basadas en el canvas y dimensiones actuales`);
                    }}
              ...
              <div className="flex justify-between items-center">
                ...
                <button
                  onClick={() => {
                    ...
                    console.log(`✅ Dimensiones autocompletadas basadas en el canvas`);
                  }}
                ...
                <div className="flex items-center gap-2 py-1 mt-2">
                  ...
                  <div 
                    ...
                    onClick={() => {
                      ...
                      console.log(`${newValue ? '🔒' : '🔓'} Relación de aspecto ${newValue ? 'mantenida' : 'libre'}`);
                    }}
                  >
                    <div 
