        ...
        if (!isEssential || (isEssential && isInContainer)) {
          return (
            <button
              onClick={(e) => {
                e.stopPropagation();
                console.log(`🗑️ Click en botón eliminar/extraer: ${component.id}`, { isEssential, isInContainer });
                
                if (isEssential && isInContainer) {
                  if (onUnattach) {
                    console.log(`🔄 Ejecutando unattach para ${component.id}`);
                    onUnattach(component.id);
                  } else {
                    console.log(`⚠️ Sin onUnattach, usando onDelete para ${component.id}`);
                    onDelete(component.id);
                  }
                } else if (onDelete) {
                  console.log(`🗑️ Eliminando componente ${component.id}`);
                  onDelete(component.id);
                }
              }}
...
// Función mejorada para obtener dimensiones de referencia
const getReferenceSize = () => {
  try {
    ...
    // Para componentes raíz, usar el banner/canvas
...
['maxWidth', 'minWidth', 'maxHeight', 'minHeight'].forEach(prop => {
  ...
  const handleContainerDrop = useCallback((e) => {
    e.preventDefault();
    // NO stopPropagation para permitir debugging
    
    // DEBUG:(`🔴 DROP en ComponentRenderer: ${component.type} (${component.id})`);
    ...
    const dragData = window.__dragData;
    // DEBUG:('  - DragData:', dragData);
    // DEBUG:('  - DropValidation:', dropValidation);
    
    if (!dragData || !dropValidation.isValid) {
      // DEBUG:('  - No hay dragData o validación inválida');
      setIsDragOver(false);
      return;
    }
        ...
        <div 
          ...
          onContextMenu={(e) => {
            ...
            removeOption.onclick = () => {
              ...
              console.log(`🔍 Verificando componente: ${child.id}`, {
                type: child.type,
                action: child.action?.type,
                isEssential,
                isEssentialByAction,
                isEssentialById
              });
              ...
              if (isEssential) {
                console.log(`⚠️ Componente esencial detectado: ${child.id}. Usando unattachFromContainer...`);
                if (onUnattach) {
                  // Pasar también el ID del contenedor padre para mejor posicionamiento
                  onUnattach(child.id, component.id);
                } else if (onRemoveChild) {
                  console.log(`⚠️ No se encontró onUnattach, usando onRemoveChild como fallback`);
                  onRemoveChild(child.id);
                }
              } else if (onRemoveChild) {
                // Para componentes no esenciales, usar la función normal
                console.log(`✅ Componente no esencial: ${child.id}. Usando función estándar removeChild...`);
                onRemoveChild(child.id);
              }
      ...
      content = (
        <div
          ...
          style={{
            ...
            margin: 0,
            padding: 0,
            lineHeight: 0, // Eliminar espacio extra debajo de imágenes
            fontSize: 0,   // Eliminar espacio entre elementos inline
  ...
  const handleGenericDragOver = useCallback((e) => {
    e.preventDefault();
    // DEBUG:(`🟢 DRAG OVER genérico en: ${component.type} (${component.id})`);
  }, [component]);
  
  const handleGenericDrop = useCallback((e) => {
    e.preventDefault();
    
    const dragData = window.__dragData;
    // DEBUG:(`🟣 DROP genérico en: ${component.type} (${component.id})`);
    // DEBUG:('  - DragData:', dragData);
    
    // Si es un contenedor, usar los handlers específicos
    if (component.type === 'container') {
      // DEBUG:('  - Es contenedor, delegando a handleContainerDrop');
      handleContainerDrop(e);
    } else {
      // DEBUG:('  - No es contenedor, solo loguear');
    }
  }, [component, handleContainerDrop]);
