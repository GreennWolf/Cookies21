...
const ImageUploader = ({ componentId, currentImage, onImageUpdate, disabled = false }) => {
  ...
  const handleImageLoad = (e) => {
    ...
    if (img.naturalWidth && img.naturalHeight && img.naturalHeight > 0) {
      ...
      console.log(`🔍 Aspect ratio detectado para imagen en ${componentId}: ${ratio.toFixed(2)}`);
    }
  };
  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = () => {
      ...
      console.log(`📸 CLIENTE: Archivo de imagen seleccionado para componente ${componentId}:`, {
        name: file.name,
      ...
      img.onload = () => {
        ...
        if (img.width > 0 && img.height > 0) {
          ...
          console.log(`📏 CLIENTE: Aspect ratio calculado para ${imageRef}: ${ratio.toFixed(2)}`);
        }
      };
      img.src = base64;
      ...
      console.log(`✅ CLIENTE: Archivo renombrado para facilitar asociación: ${renamedFile.name}`);
      console.log(`🔄 CLIENTE: Patrón de búsqueda esperado en servidor: IMAGE_REF_${componentId}_`);
      ...
      if (onImageUpdate) {
        ...
        console.log(`✅ CLIENTE: Imagen lista para guardar: ${imageRef} - Asociada con componente: ${componentId}`);
        console.log(`ℹ️ CLIENTE: Nombre para búsqueda en servidor: IMAGE_REF_${componentId}_`);
      }
    };
    reader.onerror = () => {
