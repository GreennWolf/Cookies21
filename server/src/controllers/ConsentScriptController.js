...
generateScript = catchAsync(async (req, res) => {
  ...
  generateInstallationCode = catchAsync(async (req, res) => {
    ...
    // Generar URL del script
    const scriptUrl = `${baseUrl}/api/v1/consent/script/${domainId}/embed.js`;
...
detectCountry = catchAsync(async (req, res) => {
   ...
   */
  _enhanceScriptWithDirectListeners(script, domainId, baseUrl = process.env.BASE_URL) {
  ...
  logInteraction = catchAsync(async (req, res) => {
    ...
    // Registrar analítica básica (existente)
    const analyticsService = require('../services/analytics.service');
