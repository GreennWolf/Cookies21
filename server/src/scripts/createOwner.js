...
require('dotenv').config({ path: require('path').join(__dirname, '../../.env') }); // Cargar variables de entorno desde el archivo .env
...
if (!process.env.MONGODB_URI) {
  ...
  console.error('Asegúrate de tener un archivo .env en el directorio server con MONGODB_URI=mongodb://...');
  process.exit(1);
}
...
mongoose.connect(process.env.MONGODB_URI)
  .then(async () => {
    ...
    try {
      ...
      const email = process.env.OWNER_EMAIL || 'juanmanuelrc@live.com.ar';
